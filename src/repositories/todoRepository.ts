...
export class LocalStorageTodoRepository implements TodoRepository {
  async getByDate(userId: string, date: string): Promise<Todo[]> {
    ...
    return (data.todos as Todo[]).filter((todo) => todo.userId === userId && todo.date === date);
  }

  async getAll(userId: string): Promise<Todo[]> {
    const data = readStorageData();
    return (data.todos as Todo[]).filter((todo) => todo.userId === userId);
  }

  async saveTodo(todo: Todo): Promise<Todo> {
    const data = readStorageData();
    data.todos = [...(data.todos as Todo[]), todo];
  ...
  async getById(userId: string, todoId: string): Promise<Todo | undefined> {
    const data = readStorageData();
    return (data.todos as Todo[]).find((todo) => todo.userId === userId && todo.id === todoId);
  }

  async updateTodo(todo: Todo): Promise<Todo> {
    const data = readStorageData();
    const nextTodos = (data.todos as Todo[]).map((item) => (item.id === todo.id ? todo : item));
